...
class LoadBatchAdataCross:
    ...
    def concatenate_slices(self):
        ...
        highly_variable_genes_set = set(
            self.adata_list[0]
            .var["highly_variable"][self.adata_list[0].var["highly_variable"]]
            .index
        )
        ...
        for adata in self.adata_list[1:]:
            ...
            highly_variable_genes_set = highly_variable_genes_set.intersection(
                current_set
            )
        ...
        for adata in self.adata_list:
            ...
            if isinstance(adata_Vars.X, csc_matrix) or isinstance(
                adata_Vars.X, csr_matrix
            ):
                feat = adata_Vars.X.toarray()[:]
            else:
                feat = adata_Vars.X[:]
    ...
    def construct_whole_graph(self):
        ...
        mask_list = [
            np.ones_like(i.obsm["local_graph"], dtype=int) for i in self.adata_list
        ]
    ...
    def load_data(self):
        for i in self.file_list:
            ...
            adata = sc.read_visium(
                load_path, count_file="filtered_feature_bc_matrix.h5", load_images=True
            )
            ...
            if self.label:
                df_meta = pd.read_csv(
                    os.path.join(load_path, "truth.txt"), sep="\t", header=None
                )
                df_meta_layer = df_meta[1]
